...
Processes SSTable metadata files to compute per-level statistics including key density (KD),
inter-SST gaps, and entry distribution parameters.

for icl_input

Input:
    - A directory containing per-run CSVs (generated by sstable2csv.py) with columns:
      level, id, size, entry_n, min_seq, max_seq, min_key, max_key

Output:
    - A single wide CSV file with one row per input file, containing aggregated 
      metrics (mean, std, quartiles, fits) for each level (L1, L2, etc.).
...
def fit_weibull_params(values: np.ndarray) -> Tuple[float, float, float]:
    """Fit Weibull (weibull_min) distribution; return (c, loc, scale)."""
    x_safe = values[values > 0]
    if x_safe.size < 20:
        return (np.nan, np.nan, np.nan)
    try:
...
def find_spike_range(values: np.ndarray, window_width: float = 1000.0) -> Tuple[float, float, float, int]:
    """Find the most dense range [min, max] of width window_width. Returns (min, max, ratio, count)."""
    if values.size == 0:
...
def summarize_one(csv_path: str) -> Tuple[Dict[str, Any], Set[int]]:
    ...
    for lvl in sorted(levels_present):
        ...
        if lvl == 1:
            ...
            # Fisk for intermediate and last levels
            kd_model = "fisk"
        ...
        # entry_n stats and fitting
        en = pd.to_numeric(sub["entry_n"], errors="coerce")
        en_vals = en.dropna().to_numpy(dtype=float)
        en_vals = en_vals[en_vals > 0]
        en_mean = float(np.nanmean(en_vals)) if en_vals.size > 0 else np.nan
        en_std = float(np.nanstd(en_vals)) if en_vals.size > 0 else np.nan
        en_min = float(np.nanmin(en_vals)) if en_vals.size > 0 else np.nan
        en_max = float(np.nanmax(en_vals)) if en_vals.size > 0 else np.nan
        en_q1, en_q2, en_q3 = q1_q2_q3(en_vals)
        
        # Spike & Tail analysis for entry_n
        en_spike_min, en_spike_max, en_spike_ratio, en_spike_count = find_spike_range(en_vals, window_width=1000)
        mask_spike = (en_vals >= en_spike_min) & (en_vals <= en_spike_max)
        en_tail_vals = en_vals[~mask_spike]
        en_tail_count = len(en_tail_vals)
        
        # Fit to tail data
        en_w_shape, en_w_loc, en_w_scale = fit_weibull_params(en_tail_vals)
        ...
        res[f"{prefix}_entry_mean"] = en_mean
        res[f"{prefix}_entry_std"] = en_std
        res[f"{prefix}_entry_min"] = en_min
        res[f"{prefix}_entry_max"] = en_max
        res[f"{prefix}_entry_q1"] = en_q1
        res[f"{prefix}_entry_q2"] = en_q2
        res[f"{prefix}_entry_q3"] = en_q3
        res[f"{prefix}_entry_spike_min"] = en_spike_min # 밀집 구간의 시작 ex) 63500
        res[f"{prefix}_entry_spike_max"] = en_spike_max # 밀집 구간의 끝 ex) 64500
        res[f"{prefix}_entry_spike_ratio"] = en_spike_ratio # 전체 대비 밀집 구간 비율 ex) 0.45
        res[f"{prefix}_entry_spike_count"] = en_spike_count # 밀집 구간 내 데이터 개수 ex) 450
        res[f"{prefix}_entry_tail_count"] = en_tail_count # 꼬리 구간 데이터 개수 ex) 550
        res[f"{prefix}_entry_weibull_shape"] = en_w_shape # 와이블 분포 shape 파라미터
        res[f"{prefix}_entry_weibull_loc"] = en_w_loc # 와이블 분포 loc 파라미터
        res[f"{prefix}_entry_weibull_scale"] = en_w_scale # 와이블 분포 scale 파라미터
